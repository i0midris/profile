...
@section('title', __('frontend.about_us') . ' - ' . ($settings->company_name ?? __('frontend.company_fallback')))
@section('meta_description', $about?->content ? Str::limit($about->content, 160) : __('frontend.learn_about_story'))

@section('content')
    <!-- Page Header -->
    <section class="py-24 relative overflow-hidden">
        <div class="absolute inset-0">
            <div class="absolute top-1/4 {{ app()->isLocale('ar') ? '-right-20' : '-left-20' }} w-96 h-96 bg-primary-500/10 rounded-full blur-3xl"></div>
        </div>

        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <div class="text-center" data-aos="fade-up">
                <h1 class="text-4xl md:text-5xl font-bold text-white mb-4">{{ __('frontend.about_us') }}</h1>
                <p class="text-white/60 max-w-2xl mx-auto">{{ __('frontend.learn_about_story') }}</p>
            </div>
        </div>
    </section>
                ...
                @if($about->mission_title || $about->vision_title)
                    <div class="grid md:grid-cols-2 gap-8 mb-24">
                        @if($about->mission_title)
                            <div class="glass global-card rounded-2xl p-8 h-full flex flex-col hover-lift" data-aos="fade-up">
                                ...
                                <h3 class="global-card-title text-2xl font-bold text-white mb-4">{{ $about->mission_title }}</h3>
                                <p class="global-card-copy text-white/70 leading-relaxed">{{ $about->mission_content }}</p>
                            </div>
                        @endif

                        @if($about->vision_title)
                            <div class="glass global-card rounded-2xl p-8 h-full flex flex-col hover-lift" data-aos="fade-up" data-aos-delay="100">
                                <div class="global-card-icon w-14 h-14 rounded-xl gradient-primary flex items-center justify-center mb-6">
                                    <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <h3 class="global-card-title text-2xl font-bold text-white mb-4">{{ $about->vision_title }}</h3>
                                <p class="global-card-copy text-white/70 leading-relaxed">{{ $about->vision_content }}</p>
                            </div>
                        @endif
                    </div>
                @endif
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                            @if($about->years_experience)
                                <div>
                                    ...
                                    <p class="text-white/60">{{ __('frontend.years_experience') }}</p>
                                </div>
                            @endif
                            @if($about->projects_completed)
                                <div>
                                    ...
                                    <p class="text-white/60">{{ __('frontend.projects_completed') }}</p>
                                </div>
                            @endif
                            @if($about->happy_clients)
                                <div>
                                    ...
                                    <p class="text-white/60">{{ __('frontend.happy_clients') }}</p>
                                </div>
                            @endif
                            @if($about->team_members)
                                <div>
                                    <p class="text-4xl md:text-5xl font-bold gradient-text mb-2">
                                        <span data-counter-target="{{ $about->team_members }}">{{ $about->team_members }}</span>+
                                    </p>
                                    <p class="text-white/60">{{ __('frontend.team_members') }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
        ...
        <section class="py-24 border-t border-white/10">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center mb-16" data-aos="fade-up">
                    <h2 class="text-3xl md:text-4xl font-bold text-white mb-4">{{ __('frontend.meet_our_team') }}</h2>
                    <p class="text-white/60 max-w-2xl mx-auto">{{ __('frontend.talented_people') }}</p>
                </div>
...
{{ __('frontend.view_all_team_members') }}
