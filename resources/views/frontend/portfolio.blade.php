...
@section('title', __('frontend.portfolio') . ' - ' . ($settings->company_name ?? __('frontend.company_fallback')))
@section('meta_description', __('frontend.explore_recent_projects'))

@section('content')
    <!-- Page Header -->
    <section class="py-24 relative overflow-hidden">
        <div class="absolute inset-0">
            <div class="absolute top-1/4 {{ app()->isLocale('ar') ? '-left-20' : '-right-20' }} w-96 h-96 bg-accent-500/10 rounded-full blur-3xl"></div>
        </div>

        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <div class="text-center" data-aos="fade-up">
                <h1 class="text-4xl md:text-5xl font-bold text-white mb-4">{{ __('frontend.our_portfolio') }}</h1>
                <p class="text-white/60 max-w-2xl mx-auto">{{ __('frontend.explore_recent_projects') }}</p>
            </div>
        </div>
    </section>
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($portfolios as $index => $portfolio)
                    <div x-show="activeCategory === 'all' || activeCategory === '{{ $portfolio->category }}'"
                        ...
                        <div class="glass global-card rounded-2xl overflow-hidden hover-lift">
...
{{ __('frontend.view_project') }}
                            ...
                            <div class="p-6">
                                ...
                                @endif
                                <h3 class="global-card-title text-xl font-semibold text-white mt-1 mb-2">{{ $portfolio->title }}</h3>
                                @if($portfolio->description)
                                    <p class="global-card-copy text-white/60 text-sm line-clamp-2">{{ $portfolio->description }}</p>
                                @endif
                                @if($portfolio->client_name)
                                    <p class="text-white/40 text-sm mt-3">{{ __('frontend.client') }}: {{ $portfolio->client_name }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="col-span-full text-center py-12">
                        <p class="text-white/50">{{ __('frontend.no_portfolio_items') }}</p>
                    </div>
                @endforelse
            </div>
        </div>
