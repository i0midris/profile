...
@section('title', __('admin.team_members'))
@section('subtitle', __('admin.manage_team'))
...
@section('content')
    ...
    <div class="admin-card p-4 mb-6">
        <form method="POST" action="{{ route('admin.page-visibility.update', 'team') }}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="admin-card p-6 hover-lift">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        @if($member->is_active)
                            <span
                                class="inline-flex items-center mt-2 px-2 py-0.5 rounded-full text-xs bg-green-500/20 text-green-400">{{ __('admin.active') }}</span>
                        @else
                            <span
                                class="inline-flex items-center mt-2 px-2 py-0.5 rounded-full text-xs bg-gray-500/20 text-gray-400">{{ __('admin.inactive') }}</span>
                        @endif
                    </div>
                </div>
        ...
        @empty
            <div class="col-span-full text-center py-12 text-white/50">
                {{ __('admin.no_team_members_found') }} <a href="{{ route('admin.team.create') }}"
                    class="text-primary-400 hover:underline">{{ __('admin.add_first_team_member') }}</a>
            </div>
        @endforelse
    </div>
