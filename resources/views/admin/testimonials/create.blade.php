    ...
    <div class="max-w-5xl">
        <form method="POST"
                        ...
                        <select id="rating" name="rating" required class="form-input">
                            @for($i = 5; $i >= 1; $i--)
                                <option value="{{ $i }}" {{ old('rating', $testimonial->rating ?? 5) == $i ? 'selected' : '' }}>
                                    {{ __('admin.star_rating', ['count' => $i]) }}</option>
                            @endfor
                        </select>
                    </div>
                </div>
            ...
            <div class="flex items-center gap-4">
                <button type="submit"
                    class="px-6 py-3 rounded-lg btn-primary text-white font-medium">{{ isset($testimonial) ? __('admin.update_testimonial') : __('admin.add_testimonial') }}</button>
