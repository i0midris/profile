...
@section('subtitle', __('admin.manage_client_testimonials'))
...
<div class="admin-card p-4 mb-6">
    <form method="POST" action="{{ route('admin.page-visibility.update', 'testimonials') }}"
            ...
            <div class="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
                <div class="flex items-center gap-2">
                    ...
                    @if($testimonial->is_active)
                        <span class="px-2 py-0.5 rounded-full text-xs bg-green-500/20 text-green-400">{{ __('admin.active') }}</span>
                    @else
                        <span class="px-2 py-0.5 rounded-full text-xs bg-gray-500/20 text-gray-400">{{ __('admin.inactive') }}</span>
                    @endif
                </div>
                <div class="flex items-center gap-1">
                    ...
                    <form method="POST" action="{{ route('admin.testimonials.destroy', $testimonial) }}" class="inline" onsubmit="return confirm('{{ __('admin.are_you_sure') }}')">
                        @csrf
                        @method('DELETE')
...
{{ __('admin.no_testimonials_found') }} <a href="{{ route('admin.testimonials.create') }}" class="text-primary-400 hover:underline">{{ __('admin.add_your_first_testimonial') }}</a>
        </div>
    @endforelse
</div>
