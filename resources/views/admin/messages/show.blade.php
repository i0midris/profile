...
@section('subtitle', __('admin.from_name', ['name' => $message->name]))
    ...
    <div class="max-w-3xl">
        <div class="admin-card p-6 mb-6">
            <div class="flex items-start justify-between mb-6">
                ...
                <div class="{{ app()->isLocale('ar') ? 'text-left' : 'text-right' }}">
            ...
            @if($message->subject)
                <div class="mb-4">
                    <span class="text-white/50 text-sm">{{ __('admin.subject_label') }}</span>
            ...
            @if(!$message->is_replied)
                <form method="POST" action="{{ route('admin.messages.replied', $message) }}">
                    ...
                    <button type="submit" class="px-6 py-3 rounded-lg btn-primary text-white font-medium">
                        {{ __('admin.mark_as_replied') }}
                    </button>
                </form>
            @else
                <span class="px-4 py-2 rounded-lg bg-green-500/20 text-green-400">
                    {{ __('admin.replied_on', ['date' => $message->replied_at->format('M d, Y')]) }}
                </span>
            @endif
