...
@section('subtitle', __('admin.view_manage_contact_submissions'))

@section('content')
    <div class="admin-card p-4 mb-6">
        <form method="POST" action="{{ route('admin.page-visibility.update', 'contact') }}"
    ...
    <div class="table-container overflow-hidden">
        <table class="w-full">
            <thead class="table-header">
                <tr>
                    <th class="px-6 py-4 {{ app()->isLocale('ar') ? 'text-right' : 'text-left' }} text-xs font-medium text-white/50 uppercase tracking-wider">{{ __('admin.from') }}</th>
                    <th class="px-6 py-4 {{ app()->isLocale('ar') ? 'text-right' : 'text-left' }} text-xs font-medium text-white/50 uppercase tracking-wider">{{ __('admin.subject') }}</th>
            ...
            <tbody class="divide-y divide-white/5">
                @forelse($messages as $message)
                    <tr class="hover:bg-white/5 transition-colors {{ !$message->is_read ? 'bg-primary-500/5' : '' }}">
                        ...
                        <td class="px-6 py-4">
                            <p class="text-white/80">{{ $message->subject ?? __('admin.no_subject') }}</p>
                            <p class="text-white/40 text-sm line-clamp-1">{{ Str::limit($message->message, 50) }}</p>
                        </td>
                        <td class="px-6 py-4">
                            @if($message->is_replied)
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-500/20 text-green-400">{{ __('admin.replied') }}</span>
                            @else
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-500/20 text-yellow-400">{{ __('admin.pending') }}</span>
                            @endif
                        </td>
