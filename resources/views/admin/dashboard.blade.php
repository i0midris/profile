...
@section('subtitle', __('admin.welcome_back', ['name' => auth()->user()->name]))
...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-white/50 text-sm">{{ __('admin.total_services') }}</p>
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-white/50 text-sm">{{ __('admin.team_members') }}</p>
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-white/50 text-sm">{{ __('admin.portfolio_items') }}</p>
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-white/50 text-sm">{{ __('admin.unread_messages') }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="admin-card p-6">
            <h2 class="text-lg font-semibold text-white mb-4">{{ __('admin.quick_actions') }}</h2>
            <div class="grid grid-cols-2 gap-4">
                <a href="{{ route('admin.services.create') }}"
                    ...
                    <span class="text-white/80 text-sm">{{ __('admin.add_service') }}</span>
                </a>
                <a href="{{ route('admin.team.create') }}"
                    ...
                    <span class="text-white/80 text-sm">{{ __('admin.add_team_member') }}</span>
                </a>
                <a href="{{ route('admin.portfolio.create') }}"
                    ...
                    <span class="text-white/80 text-sm">{{ __('admin.add_portfolio') }}</span>
                </a>
                <a href="{{ route('admin.settings.edit') }}"
                    ...
                    <span class="text-white/80 text-sm">{{ __('admin.edit_settings') }}</span>
                </a>
            </div>
        </div>
        ...
        <div class="admin-card p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-white">{{ __('admin.recent_messages') }}</h2>
                <a href="{{ route('admin.messages.index') }}"
                    class="text-primary-400 hover:text-primary-300 text-sm">{{ __('admin.view_all') }}</a>
            </div>
            <div class="space-y-4">
                @forelse($recentMessages as $message)
                    <a href="{{ route('admin.messages.show', $message) }}"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-white/50 text-xs truncate">{{ $message->subject ?? __('admin.no_subject') }}</p>
                            <p class="text-white/30 text-xs mt-1">{{ $message->created_at->diffForHumans() }}</p>
                        </div>
                    </a>
                @empty
                    <p class="text-white/50 text-center py-4">{{ __('admin.no_messages_yet') }}</p>
                @endforelse
            </div>
        </div>
    </div>
